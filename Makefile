...
install:
	...
	install -D -m644 -t ${DESTDIR}/${man1dir} ${MAN1:%=docs/%.1}
	install -D -m644 -t ${DESTDIR}/${man5dir} ${MAN5:%=docs/%.5}
...
install-tar:
	...
	install -D -m644 -t ${DESTDIR}/etc \
		...
		etc/runmirrors.conf.sample \
		etc/runmirrors.mirror.sample
	install -D -m644 -t ${DESTDIR}/doc ${MAN1:%=docs/%.1.md} ${MAN5:%=docs/%.5.md}
...
ctx-tarball:
	$(MAKE) clean all
	rm -rf debian/ctx-tarball
	$(MAKE) install-tar DESTDIR=debian/ctx-tarball
	rm -f debian/ctx-tarball/etc/ftpsync.conf.sample
	install -D -m644 -t debian/ctx-tarball/etc \
		etc/ftpsync-ctx.conf.sample \
		etc/mirror-ctx-images.conf.sample
	install -D -m755 -t debian/ctx-tarball/bin \
		bin/mirror-ctx-images
	install -d debian/ctx-tarball/log
	cd debian/ctx-tarball && tar --sort=name --owner=root:0 --group=root:0 -zcf ../../ftpsync.tar.gz *
	rm -rf debian/ctx-tarball
